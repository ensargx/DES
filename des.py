...
class DES:
    ...
    def generate_subkeys(self):
        ...
        return [self._permute(self.initial_key, self._key_permutation_table(i)) for i in range(16)]

    def _key_permutation_table(self, round):
        ...
        # Permute the key using the key permutation table
        key = self._permute(self.initial_key, key_permutation_table)

        # Split the key into left and right parts
        left, right = key[:28], key[28:]

        # Left shift the left and right parts
        for i in range(left_shift_table[round]):
            left = self._left_shift(left)
            right = self._left_shift(right)
